...
| Field | Value |
|-------|-------|
| **Depends On** | ADR-0002 (couches de collision - definit les layers/masks utilises par S02) |
| **Enables** | Aucun |
| **Blocks** | Epic S02 — Saisie et lancer (ne peut pas commencer l'implémentation sans ce ADR Accepted) |
| **Ordering Note** | Doit être Accepted après ADR-0002 et avant la création des stories S02 |
...
- Doit lire les propriétés d'objet via S05 (entry `catalogue_entry` portée par l'objet)
- Doit émettre les signaux `grab_performed`, `throw_performed`, `melee_performed` consommés par S08, S04, S11
- Doit recevoir la direction du yaw caméra (S10) pour orienter le lancer
...
│     @export var camera_pivot: Node3D      ← injecté par Player scene (S10)
...
@export var camera_pivot: Node3D      # Source du yaw pour le lancer (S10)
...
- **Raison du rejet** : Viole le principe de testabilite des coding standards ("toutes les methodes publiques doivent etre testables en unitaire via injection de dependances") et cree une dependance cachee que tout systeme peut appeler sans declaration explicite
...
- **Raison du rejet** : Fragile face aux refactorings de scene ; meme probleme de testabilite que l'Autoload
...
| saisie-lancer.md | S02 responsable du cast float→int avant appel S06 | `melee_strike()` / `throw_object()` castent les valeurs issues de `held_object.catalogue_entry` en int avant d'émettre |
