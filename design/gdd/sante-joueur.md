...
2. **Réception des dégâts** : S07 expose `receive_damage(amount: int, type: DamageType)`. L'appelant (S09 — IA ennemie, via collision ou attaque) fournit un entier ≥ 1 (DAMAGE_MIN garanti par S06). S07 ne recalcule pas les dégâts.

3. **Invincibilité temporaire (I-frames)** : après avoir reçu un coup, le joueur est invincible pendant `I_FRAME_DURATION` (0,5 s). Tout appel à `receive_damage()` pendant cette fenêtre est ignoré silencieusement. Les I-frames s'appliquent indépendamment de `DamageType`.
...
5. **Émission des signaux** : après chaque réduction valide de HP, S07 émet :
   ...
   - `player_hit(damage: int, type: DamageType, current_hp: int)` → S11, S13 (feedback visuel/audio)
...
| `DEAD` | Joueur mort, HP = 0, signaux émis | → `ALIVE` uniquement via reset de session (S11 appelle `reset()`) |
...
**Transition DEAD → ALIVE** : S07 expose `reset()`. S11 l'appelle pour préparer le retry. `reset()` restore `current_hp = HP_JOUEUR_MAX` et l'état passe à `ALIVE`.
...
| **S09** (IA ennemie) | → S07 | `receive_damage(amount: int, type: DamageType)` | Appel direct sur le nœud HealthSystem du joueur (@export injecté) |
| **S11** (État global) | ← S07 | `player_died()`, `player_hit(...)` | Signaux GDScript |
| **S11** (État global) | → S07 | Appel `reset()` au retry | Appel direct |
...
**EC-05 — reset() appelé en état ALIVE**
S11 appelle `reset()` alors que le joueur est encore vivant (ex. bug de séquencement). → `current_hp` repart à `HP_JOUEUR_MAX`, état reste `ALIVE`. Pas d'effet négatif — le reset est idempotent.

**EC-06 — Deux ennemis frappent simultanément (même frame)**
...
| **S09 — IA ennemie** | Appelle `receive_damage(amount, type)` lors des contacts ou attaques. S07 suppose que S09 ne fait jamais d'appels en rafale sans délai (délai géré par I-frames). |
| **S11 — Gestionnaire d'état** | Appelle `reset()` pour préparer le retry. S11 est responsable du séquencement (ne pas appeler reset() avant player_died()). |

### Dépendances sortantes (systèmes qui dépendent de S07)

| Système | Ce qu'il reçoit de S07 |
|---|---|
| **S11 — Gestionnaire d'état** | `player_died()` — déclenche le game-over et orchestre le retry. `player_hit(damage, type, current_hp)` — peut déclencher des effets de caméra ou de feedback global. |
| **S13 — HUD** | `player_hp_changed(current_hp, max_hp)` — met à jour l'affichage HP. `player_hit(damage, type, current_hp)` — déclenche l'animation de dégât sur le HUD. |
...
|---|---|---|
| `player_hit(damage, type, current_hp)` | S13 (HUD) | Flash rouge de l'écran, animation de la barre HP |
| `player_hit(damage, type, current_hp)` | S11 | Shake de caméra (optionnel, calibré au prototype) |
...
**AC-02** — `receive_damage(5, MELEE)` depuis l'état ALIVE (hors I-frames) réduit `current_hp` de 5. Vérifiable : test unitaire sur le nœud S07 isolé.
...
**AC-07** — Après `reset()`, `current_hp == HP_JOUEUR_MAX` (20) et l'état est `ALIVE`. Vérifiable : test unitaire.
