...
S09 gère le comportement de chaque instance d'ennemi individuelle : navigation vers le joueur, pression de proximité, et hébergement du sous-système de santé S08. Chaque ennemi est une scène autonome dont la racine est un `CharacterBody3D` (layer 4). À l'instanciation, S03 injecte `player` et `wave_manager` via `@export` (ADR-0009), et S09 câble les signaux sortants de S08 : `enemy_died(enemy)` vers S03, et `enemy_hit(...)` vers S15 (V1.0) si présent. En MVP, le comportement reste volontairement simple : se déplacer vers le joueur, s'arrêter à portée, et infliger un **dégât au contact** (DamageType `ENEMY_MELEE`) permettant le game-over (S07) sans système d'attaque complexe. La cadence effective des dégâts est régulée par les i-frames de S07.
...
1. **Structure de l'ennemi** : chaque ennemi est une scène autonome dont la racine est un `CharacterBody3D` (layer 4, mask 1|3|4 — ADR-0002). La scène contient un nœud `EnemyAI` (S09) et un nœud `EnemyHealth` (S08), ainsi qu'un `NavigationAgent3D` (où qu'il soit dans le sous-arbre, au choix). Le contrat de composition est formalisé par ADR-0009.

2. **Injection de dépendances au spawn** : le spawner (S03 — WaveManager) instancie l'ennemi (`instantiate()`), injecte les dépendances via `@export` — `player: Node3D`, `wave_manager: Node` (S03) — puis appelle `add_child()`. `_ready()` se déclenche après l'injection et câble immédiatement les signaux de S08 vers S03 (et vers S15 en V1.0).
...
5. **Distance d'arrêt** : `STOPPING_DISTANCE = 0.8 m` centre-à-centre. `target_desired_distance` du `NavigationAgent3D` est configuré à cette valeur. Quand l'ennemi est dans la stopping distance, il cesse de naviguer et reste en place.
...
9. **Dégât au contact (MVP minimal)** : quand l'ennemi est en contact avec le joueur (collision layer 2) et à portée (≤ STOPPING_DISTANCE), il tente d'infliger un coup “contact” via :
   - `damage = DamageCalculator.calculate(ENEMY_DAMAGE_BASE, 1.0, DamageCalculator.DamageType.ENEMY_MELEE)`
   - appel `player.receive_damage(damage, DamageCalculator.DamageType.ENEMY_MELEE)`

   `ENEMY_DAMAGE_BASE = 3` par défaut (tuning knob). Les i-frames de S07 garantissent qu'un “collage” ne produit pas de multi-hit injuste.
...
| **S03** (Vagues) | S03 → S09 au spawn | Injection @export `player` et `wave_manager` | Instanciation + injection avant `add_child()` |
| **S03** (Vagues) | ← S08 | `enemy_died(enemy)` | Signal `enemy_died` consommé par S03 pour décrémenter `enemies_alive` |
| **S07** (Santé joueur) | ← S09 | `receive_damage(damage, ENEMY_MELEE)` en contact | Appel direct sur `player.receive_damage(...)` |
...
| **S08 — Santé ennemie** | S09 héberge S08 comme nœud enfant. S09 connecte `enemy_died` de S08 vers S03 (wave_manager) dans `_ready()` (et `enemy_hit` vers S15 en V1.0). S08 appelle `queue_free()` à la mort de l'ennemi — S09 ne gère pas ce retrait lui-même. |
| **S03 — Vagues d'ennemis** (spawner) | Injecte les dépendances (`wave_manager`, `player`) avant `add_child()`. S03 est responsable du cycle de vie de l'instance S09. |
...
| **S15 — VFX/Audio (V1.0)** | `enemy_hit(damage, type, current_hp)` via S08 → S15 déclenche les feedbacks d'impact. |
| **S02 — Saisie/Lancer** | Détecte la collision avec la couche physique de S09 (layer 4) pour appeler `receive_damage()` sur S08. |

### Note sur la circularité apparente S09 ↔ S03

S03 (vagues) spawn les instances S09, et S09 émet `enemy_died` vers S03. Ce n'est pas une dépendance circulaire de conception : S03 ne dépend pas de S09 pour son initialisation — il reçoit seulement des événements de S09 après le spawn. Le contrat est unidirectionnel à l'exécution : S03 crée, S09 notifie.
...
| `ENEMY_DAMAGE_BASE` | 3 | 1 – 6 | Dégâts bruts du “contact” (avant S06). Valeur faible par design : la menace vient de l'occupation d'espace, pas d'un burst. Doit rester compatible avec S07 (`I_FRAME_DURATION = 0.5 s`) pour éviter une mort instantanée au contact. |
...
| **Feedback de mort** | S15 gérera l'effet visuel (hors scope S09) — S08 émet `enemy_died(enemy)` (câblé par S09). |
| **Feedback d'impact** | S15 gérera (hors scope S09) — S08 émet `enemy_hit(...)` (câblé par S09). |
...
Deux `receive_damage()` arrivant dans la même frame amènent `current_hp` à 0 deux fois. Pass : `enemy_died` émis une seule fois (S08 garantit l'unicité — AC-04 de S08). Fail : `enemy_died` émis deux fois, double comptage de fin de vague.
...
**AC-10 — Dégât au contact (MVP)**
Un ennemi reste en contact avec le joueur (collision) pendant ≥ 1.0 s. Pass : au moins un appel `player.receive_damage(...)` est tenté côté S09 et le joueur perd effectivement des HP (S07 applique au plus un coup par fenêtre d'i-frames). Fail : aucun dégât n'est jamais infligé au joueur même en contact prolongé.
