...
3. **Contrainte de timing.** De la réception de `game_state_changed(GameState.GAME_OVER)` à l'interactivité de l'écran : **0 ms** (même frame). Le budget **≤ 3 s** concerne la séquence complète (mort → retour au jeu) portée par S11+S12 ; S13 n'ajoute aucun délai intentionnel.
...
5. **Garde d'input.** En MVP, la sortie de GAME_OVER se fait via `reload_current_scene()` (S12) : le HUD est détruit, puis recréé, ce qui réinitialise l'écran Game Over implicitement. S13 garde l'écoute `retry` active jusqu'au reload. (Si une réinitialisation « soft » remplace le reload en V1.0, alors `game_state_changed(PRE_WAVE)` devra masquer l'overlay et désactiver l'écoute `retry`.)
...
| `GAME_OVER` | `game_state_changed(GAME_OVER)` depuis S11 | `reload_current_scene()` (S12) — HUD détruit puis recréé | Éléments permanents masqués. Overlay Game Over affiché (texte + prompt retry). |

```
PLAYING ──[game_state_changed(GAME_OVER)]──> GAME_OVER
GAME_OVER ──[reload_current_scene (S12)]──> PLAYING (nouvelle instance)
```
...
|-----------|-----------|
| `PRE_WAVE` | En MVP (reload) : no-op. Le HUD vient d'être recréé après reload et n'a pas d'overlay Game Over à masquer. |
...
| `VICTORY` | No-op (pas d'écran victoire en MVP : S13 reste en PLAYING tant que la scène n'est pas rechargée/changée). |
...
(d) En MVP, le retry déclenche un `reload_current_scene()` (S12) : le HUD est détruit et l'écran Game Over disparaît implicitement. Au rechargement, le HUD est recréé dans son état initial (éléments permanents visibles, HP pleine, compteur `"1/3"`).
